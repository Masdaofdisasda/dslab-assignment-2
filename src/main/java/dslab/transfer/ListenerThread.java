...
public class ListenerThread extends Thread {
    ...
    public ListenerThread(Config config, Shell shell) {
        this.config = config;
        this.shell = shell;
        this.stopFlag = new AtomicBoolean(false);

        // could as well use FixedThreadPool
        ...
        try (ServerSocket serverSocket = new ServerSocket(port)) {
            ...
            while (!stopFlag.get()) {
                try {
                    ...
                    AcceptMessageRunnable worker = new AcceptMessageRunnable(clientSocket, runningAcceptMessageTasks);
    ...
    public void shutdown() {
        shell.out().println("Stopping all worker Threads!");
       ...
       acceptMessageThreadPool.shutdown();
        ...
        shell.out().println("All WorkerThreads stopped!");
    }
}